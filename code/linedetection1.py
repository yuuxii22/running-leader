...
def average_slope_intercept(photo,lines):
    ...
    for line in lines:
        ...
        if slope < -0.5:
            if x1 < left_region_boundary and x2 < left_region_boundary:
                left.append(np.array([slope,intercept]))
        elif slope > 0.5: 
            if x1 > right_region_boundary and x2 > right_region_boundary:
                right.append(np.array([slope,intercept]))
    ...
    if(len(left) > 0):
        try:
            leftLines = cluster(left)
        except:
            leftLines = [np.average(left, axis=0)]
        print("LL")
        print(leftLines)
        for l in leftLines:
            print(l)
            detLine.append(make_coordinate(l))
            
    if(len(right) > 0):
        try:
            rightLines = cluster(right)
        except:
            rightLines = [np.average(right, axis=0)]
        print("RL")
        print(rightLines)
        
        for r in rightLines:
            print(r)
            detLine.append(make_coordinate(r))
...
def check_hwv(image):
    ...
    return np.array(height),np.array(width),np.array(vertices)
...
if __name__ == '__main__':
    ...
    while(True):
        ret, frame = cam.read()
        height,width,vertices = check_hwv(frame)
        test_image(frame)
